...
export async function runHealthcheck(ctx, opts) {
    const checks = [];
    const envPresent = {
    ...
    if (opts.deep) {
        if (missingEnv.length > 0 || !ctx.gsc) {
            ...
            try {
                ...
                const match = sites.find((s) => s.siteUrl === configuredProp ||
        ...
        if (ctx.ai.id === 'gsc_export') {
            ...
            checks.push({
                id: 'ai_provider',
                status: 'warn',
                detail: 'Provider de IA none: la API oficial de Google Search Console v1 no expone métricas de IA generativa (revisión 20260902). Activable exportando el CSV de la UI y publicándolo con scripts/publish-ai-export.mjs (KEYTRENDS_AI_EXPORT_URL), o con KEYTRENDS_AI_EXPORT_DIR en local.',
            });
        }
    }
    let overall = 'pass';
