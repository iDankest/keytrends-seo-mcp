...
if (args.includes('--healthcheck')) {
  (async () => {
    ...
    process.stderr.write(
      `Error fatal al ejecutar healthcheck: ${err instanceof Error ? err.message : String(err)}\n`
    );
    process.exit(1);
  });
} else if (args.length > 0) {
  process.stderr.write(
    `Argumento desconocido: ${args[0]}\nUsa --help para ver las opciones disponibles.\n`
  );
  process.exit(2);
} else {
  // Stdio server mode
  ...
  console.log = (...a: unknown[]) =>
    initialLogger.warn('console.log suprimido para proteger stdout del protocolo MCP', {
      args: a.map(String),
    });
  console.info = (...a: unknown[]) =>
    initialLogger.info('console.info suprimido', {
      args: a.map(String),
    });
  console.debug = (...a: unknown[]) =>
    initialLogger.debug('console.debug suprimido', {
      args: a.map(String),
    });
