...
export interface SitemapGraph {
  ...
  sub_sitemaps: SubSitemapResult[];
  total_urls: number;
  urls: string[]; // Empty if collectUrls = false
  errors: string[];
}
...
export async function fetchRobots(opts: {
  siteUrl: string;
  http: HttpClient;
}): Promise<{ status: number; text: string; sitemaps: string[] }> {
...
export async function fetchSitemapGraph(opts: {
  ...
  let entrypointXml: string | null = null;
  ...
  for (const candidate of candidateQueue) {
    ...
    try {
      ...
      errors.push(
        `Error al acceder a ${candidate}: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  }
  ...
  const subSitemaps: SubSitemapResult[] = [];
