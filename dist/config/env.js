...
export function normalizeUrlWithTrailingSlash(rawUrl) {
    try {
...
export function deriveSiteUrl(property, explicitSiteUrl) {
...
export function loadConfig(env = process.env) {
    ...
    if (!rawProperty) {
        ...
        if (!rawProperty.startsWith('sc-domain:') &&
            ...
            warnings.push(`GSC_PROPERTY ('${rawProperty}') no comienza con 'sc-domain:' ni 'https://'; Google podría rechazarla con 404.`);
        }
    }
    if (!clientId)
    ...
    const google = clientId && clientSecret && refreshToken
    ...
    if (env.KEYTRENDS_LOG_LEVEL) {
        ...
        if (parsedLogLevel.success) {
            ...
            warnings.push(`KEYTRENDS_LOG_LEVEL inválido ('${env.KEYTRENDS_LOG_LEVEL}'); usando default 'info'.`);
        }
    }
    let aiProviderMode = 'auto';
    if (env.KEYTRENDS_AI_PROVIDER) {
        ...
        if (parsedAiMode.success) {
            ...
            warnings.push(`KEYTRENDS_AI_PROVIDER inválido ('${env.KEYTRENDS_AI_PROVIDER}'); usando default 'auto'.`);
        }
    }
    ...
    if (env.KEYTRENDS_HTTP_TIMEOUT_MS) {
        ...
        if (!isNaN(parsedTimeout) && parsedTimeout > 0) {
            ...
            warnings.push(`KEYTRENDS_HTTP_TIMEOUT_MS no numérico o inválido ('${env.KEYTRENDS_HTTP_TIMEOUT_MS}'); usando default 20000ms.`);
        }
    }
    ...
    if (env.KEYTRENDS_MAX_INSPECT_URLS) {
        ...
        if (!isNaN(parsedMax) && parsedMax > 0) {
            ...
            warnings.push(`KEYTRENDS_MAX_INSPECT_URLS no numérico o inválido ('${env.KEYTRENDS_MAX_INSPECT_URLS}'); usando default 50.`);
        }
    }
    const config = {
