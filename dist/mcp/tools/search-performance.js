...
import { measured, OFFICIAL_GSC } from '../../models/provenance.js';
import { resolveWindow } from '../../utils/dates.js';
import { registerJsonTool } from '../wrap.js';
import { windowShape } from './gsc-summary.js';
export function registerSearchPerformanceTool(server, ctx) {
    registerJsonTool(server, ctx, {
        name: 'keytrends_get_search_performance',
        title: 'Rendimiento detallado de búsqueda en Google',
        description: 'Consulta métricas agrupadas por consulta (QUERY), página (PAGE), país (COUNTRY), dispositivo (DEVICE), fecha (DATE) o apariencia (SEARCH_APPEARANCE).',
        inputSchema: {
            ...windowShape,
            dimension: z
        ...
        if (args.dimension === 'PAGE' || args.dimension === 'QUERY') {
            warnings.push('Recuento es cota inferior: Google descarta filas de baja frecuencia al agrupar por página o consulta y aplica topes de 25.000 filas por respuesta (https://developers.google.com/webmaster-tools/v1/how-tos/all-your-data).');
        }
        if (args.dimension === 'SEARCH_APPEARANCE') {
            warnings.push('searchAppearance es un catálogo de resultados enriquecidos y no contiene ningún valor de IA (https://support.google.com/webmasters/answer/17011259); no sirve para medir AI Overviews ni AI Mode.');
        }
